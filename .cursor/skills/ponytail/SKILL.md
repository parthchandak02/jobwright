---
name: ponytail
description: Force the laziest solution that works: YAGNI, reuse, stdlib, shortest diff. Intensity lite/full/ultra. Never lazy about understanding, security, or one check for non-trivial logic.
---
...
4. **Existing seam / config covers it?** Add a value to an existing config or extend an existing handler before inventing a new module, endpoint, or file. Prefer an existing contract over minting a new one.
