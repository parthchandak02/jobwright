...
name: humanizer
description: |
  Rewrites prose that sounds AI-generated while keeping the writer's facts, meaning,
  and voice. Use when drafting or editing any user-facing text: docs, README,
  cover letters, digests, CLI messages, commit bodies, handoffs, PR descriptions,
  skills, or when the user says humanize, sound less AI, or polish prose. Based on
  Wikipedia's "Signs of AI writing" (blader/humanizer v2.11.0).
license: MIT
metadata:
  version: "2.11.0"
...
The patterns below come from Wikipedia's ["Signs of AI writing"](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing), maintained by WikiProject AI Cleanup. Upstream: [blader/humanizer](https://github.com/blader/humanizer) (MIT). See [references/ATTRIBUTION.md](references/ATTRIBUTION.md).

## Proactive use (write human first)

Do not wait for a separate "humanize this" step. When this skill applies:

1. **Draft with patterns in mind.** As you write README sections, docs, cover letters, digests, CLI help, or chat replies, avoid the patterns below in the first draft.
2. **Self-check before returning.** Run the [Rewrite process](#rewrite-process) on any prose block longer than two sentences before you show it to the user.
3. **File edits.** When changing markdown or user-facing strings in code, humanize only the prose. Leave code, YAML, JSON, and link targets unchanged unless the user asks otherwise.

For jobwright-specific surfaces (cover letters, Hermes digests, wizard text), see [references/jobwright-contexts.md](references/jobwright-contexts.md).
