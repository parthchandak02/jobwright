...
API keys are written to the single global .env (repo root; see config.global_env_path).
"""
...
from jobwright.config import ensure_dirs
...
def _setup_profile() -> dict:
    ...
    console.print(Panel("[bold]Step 2: Profile[/bold]\nTell jobwright about yourself. This powers scoring, tailoring, and auto-fill."))
...
def _setup_searches() -> None:
    ...
    # Build YAML content
    lines = [
        "# jobwright search configuration",
...
def _setup_ai_features() -> None:
    ...
    if not Confirm.ask("Enable AI scoring and resume tailoring?", default=True):
        console.print("[dim]Discovery-only mode. You can configure AI later with [bold]jobwright init[/bold].[/dim]")
        return
    ...
    env_lines = ["# jobwright configuration", ""]

    if provider == "gemini":
        api_key = Prompt.ask("Gemini API key (from aistudio.google.com)")
        model = Prompt.ask("Model", default="gemini-3.7-flash")
    ...
    env_lines.append("")
    env_path = config.global_env_path()
    config.write_private_text(env_path, "\n".join(env_lines))
...
def _setup_auto_apply() -> None:
    ...
    console.print(Panel(
        ...
        "jobwright can autonomously fill and submit job applications\n"
        "using Claude Code as the browser agent."
    ))

    if not Confirm.ask("Enable autonomous job applications?", default=True):
        console.print("[dim]You can apply manually using the tailored resumes jobwright generates.[/dim]")
        return
    ...
    if Confirm.ask("Configure CapSolver API key? (optional)", default=False):
        ...
        # Append to the single global .env or create it
        env_path = config.global_env_path()
        if env_path.exists():
            ...
            config.write_private_text(env_path, f"# jobwright configuration\nCAPSOLVER_API_KEY={capsolver_key}\n")
...
def run_wizard() -> None:
    ...
    console.print(
        Panel.fit(
            "[bold green]jobwright Setup Wizard[/bold green]\n\n"
            "This will create your configuration at:\n"
            f"  [cyan]{config.APP_DIR}[/cyan]\n\n"
            "You can re-run this anytime with [bold]jobwright init[/bold].",
            border_style="green",
        )
    )
    ...
    from jobwright.config import get_tier, TIER_LABELS, TIER_COMMANDS
    ...
    unlock_hint = ""
    if tier == 1:
        unlock_hint = "\n[dim]To unlock Tier 2: configure an LLM API key (re-run [bold]jobwright init[/bold]).[/dim]"
    elif tier == 2:
