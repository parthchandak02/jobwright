...
| Surface | Behavior |
|---------|----------|
| **Auto Search** | Full prep pipeline (`discover` → `enrich` → `score` → `portfolio` → `tailor` → `cover` → `docx` → `connect`). Progress + SSE logs live in `AutoSearchControls` so log ticks do not re-render Kanban cards. Closing the dialog does not stop the run; **Stop** sends SIGTERM/SIGKILL. Attaches to in-flight runs via `GET /api/runs` (`web_runs.json`). |
