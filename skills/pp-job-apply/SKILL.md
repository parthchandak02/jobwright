...
description: >-
  ...
  ~/.jobwright or ~/.jobwright-users secrets.
version: 1.3.0
author: parthchandak
license: AGPL-3.0
...
metadata:
  hermes:
    tags: [job-search, jobwright, cursor, automation, cron, confirmation-gate, captcha, multi-profile]
...
**Legacy single-user data:** `~/.jobwright/`
**Multi-profile registry:** `~/.jobwright-users/users.yaml`
**Per-user data:** `~/.jobwright-users/<user_id>/` (`profile.json`, `resume.txt`, `searches.yaml`, `jobwright.db`)
**Global API keys:** one gitignored `.env` at the repo root, shared across all profiles (no per-user key copies)
...
2. Register:
   ...
   # API keys are global (one repo-root .env) — a new user only gets a data dir.
   # apply stays OFF unless they explicitly ask:
   # jobwright users set <id> --apply
   ```
3. Write their files into `~/.jobwright-users/<id>/`:
   ...
   - Optional: `connections.csv` (LinkedIn export) for `jobwright network`
4. Always put `--user` **before** the subcommand:
   ```bash
   jobwright --user <id> doctor
   jobwright --user <id> run discover enrich score ...
   ```
...
6. Manual run / status:
   ```bash
   jobwright --user <id> doctor
   jobwright --user <id> run discover enrich score portfolio tailor cover -w 4 --min-score 5 --validation lenient
   jobwright --user <id> status
   ```
...
jobwright users show richa
jobwright users set richa --whatsapp "whatsapp:..." --no-apply
jobwright --user richa network
jobwright --user richa targets
```
...
cd ~/projects/jobwright
jobwright doctor
# or: jobwright --user richa doctor
```
...
|-------|---------|---------|
| 1 discover | `jobwright [--user ID] run discover` | python-jobspy (Indeed, Google, ZipRecruiter  -  not LinkedIn) |
...
1. `job_apply_morning.sh` with `JOBWRIGHT_USER=<id>` via Hermes cron (`no_agent`)
2. Clears stale `APPLY_CONFIRMED` in that user's data dir
3. Runs: `jobwright --user <id> run discover enrich score portfolio tailor cover ...`
4. Digest delivered to that user's `whatsapp_target`

**User replies CONFIRM APPLY** (only if `apply_enabled`)
1. `JOBWRIGHT_USER=<id> job_apply_confirm.sh`
2. `JOBWRIGHT_USER=<id> job_apply_on_confirm.sh`  -  refuses if apply disabled
3. Report results
...
6. No secrets in git (`~/.jobwright/` and `~/.jobwright-users/` never committed)
...
Creates per-user crons `job-apply-morning-<id>`, `job-apply-digest-<id>`, `job-apply-watchdog-<id>` with wrappers that export `JOBWRIGHT_USER`. Each delivers to that user's `whatsapp_target`.
